...
app.controller('main-controller', ($scope, $http) => {
    ...
    $scope.addBook = () => {
        const url = 'api/book';
        $http.post(url, $scope.modalBook)
            .then(response => {
                $scope.catalog.push(response.data);
                swal('Book Added', `Book ${$scope.modalBook.title} added!`, "success")
            }).catch(err => {
                swal('Book Not Added', `Book ${$scope.modalBook.title} not added!`, "error")
                console.error(err);
            })
    };
    ...
    $scope.updateBook = () => {
        ...
        const index = $scope.catalog.findIndex(book => { return book.id == id });
    ...
    $scope.setEditModal = (id) => {
        ...
        $http.get(url)
            .then(response => {
                response.data.publication_date = new Date(response.data.publication_date);
                $scope.modalBook = response.data;
            }).catch(err => {
                swal('Getting Book Failed', `Book ${$scope.modalBook.title} not found!`, "error")
                console.error(err);
            })
    }
    ...
    function deleteBook(id, index) {
        ...
        $http.delete(url)
            .then((response) => {
                ...
                swal("Deleting Book Failed!", `Deleting book ${$scope.catalog[index].title} faild!`, "error");
